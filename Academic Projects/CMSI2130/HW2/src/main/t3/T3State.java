...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
        // [!] TODO
    	// make a treemap (not HashMap)
    	// get the open tiles we could take actions in - getOpenTiles
    	// get the possible moves we can take - getMoves
    	// get the next state we could have - getNextState
    	
    	Map<T3Action, T3State> allTransitions = new TreeMap<T3Action, T3State>();
    	ArrayList<int[]> openTiles = this.getOpenTiles();
    	int[] possibleMoves = this.getMoves();
    	for(int[] element : openTiles) {
    		T3Action action = new T3Action(element[0], element[1], possibleMoves[0]);
    		T3State nextState = this.getNextState(action);
    		allTransitions.put(action, nextState);
    		
    		T3Action action2 = new T3Action(element[0], element[1], possibleMoves[1]);
    		T3State nextState2 = this.getNextState(action2);
    		allTransitions.put(action2, nextState2);
    		
    		T3Action action3 = new T3Action(element[0], element[1], possibleMoves[2]);
    		T3State nextState3 = this.getNextState(action3);
    		allTransitions.put(action3, nextState3);
    	}
