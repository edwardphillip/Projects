...
public class DistlePlayer {
    ...
    public void startNewGame (Set<String> dictionary, int maxGuesses) {
    	this.allPossibleGuesses = dictionary;
    }
    ...
    public String makeGuess () {
    	Iterator<String> iter = allPossibleGuesses.iterator();
    	int index = new Random().nextInt(allPossibleGuesses.size());    	
    ...
    public void getFeedback (String guess, int editDistance, List<String> transforms) {
    	new EditDistanceUtils();
    	Set<String> newAllPossibleGuesses = new HashSet<String>();   	
    	
    	Iterator<String> iter = allPossibleGuesses.iterator();
    	for(int i = 0; i < allPossibleGuesses.size(); i++) {
    		String candidates = iter.next();
    		if(candidates.equals(guess))
    			continue;  		
    		int distance = EditDistanceUtils.editDistance(guess, candidates);
    		List<String> temp = EditDistanceUtils.getTransformationList(guess, candidates);
    		if(distance == editDistance && temp.equals(transforms))
    			newAllPossibleGuesses.add(candidates);	
    	}
    	allPossibleGuesses = newAllPossibleGuesses;
    }
