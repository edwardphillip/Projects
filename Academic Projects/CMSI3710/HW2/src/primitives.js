...
const fillRect = (context, x, y, w, h, c1, c2, c3, c4) => {
  ...
  const fillRectTwoColorsRadial = () => {
    const radius = h > w ? w / 2 : h / 2
    const centerX = x + w / 2
    const centerY = y + h / 2
    let outerColor = leftColor
    const colorDelta = [(c2[0] - c1[0]) / radius, (c2[1] - c1[1]) / radius, (c2[2] - c1[2]) / radius]

    const inside = (x, y) => {
      return radius > distanceToCenter(x, y)
    }
    ...
    for (let i = y; i < bottom; i += 1) {
      for (let j = x; j < right; j += 1) {
        if (inside(j, i)) {
          outerColor = [
            outerColor[0] + colorDelta[0] * (radius - distanceToCenter(j, i)),
            outerColor[1] + colorDelta[1] * (radius - distanceToCenter(j, i)),
            outerColor[2] + colorDelta[2] * (radius - distanceToCenter(j, i))
          ]
        }
        setPixel(context, j, i, ...outerColor)
        outerColor = leftColor
      }
    }
  }
...
const plotCirclePoints = (context, xc, yc, x, y, color, nextColor) => {
  ...
  for (let i = x; i >= -x; i--) {
    const gradientBottom = addArrays(
      color,
      addArrays(
        nextColor,
        color.map(x => -x)
      ).map(element => element * ((y + yc - 215) / 200))
    )
    const gradientTop = addArrays(
      color,
      addArrays(
        nextColor,
        color.map(x => -x)
      ).map(element => element * ((yc - y - 215) / 200))
    )
    setPixel(context, i + xc, y + yc, ...gradientBottom)
    setPixel(context, i + xc, yc - y, ...gradientTop)
  }
  ...
  for (let i = y; i >= -y; i--) {
    const gradientBottom = addArrays(
      ...
      addArrays(
        nextColor,
        color.map(x => -x)
      ).map(element => element * ((x + yc - 215) / 200))
    )
    const gradientTop = addArrays(
      color,
      addArrays(
        nextColor,
        color.map(x => -x)
      ).map(element => element * ((yc - x - 215) / 200))
    )

    setPixel(context, i + xc, x + yc, ...gradientBottom)
    setPixel(context, i + xc, yc - x, ...gradientTop)
  }
}
...
const circleTrig = (context, xc, yc, r, color1, color2) => {
  const theta = 1 / r
  ...
  while (x >= y) {
    plotCirclePoints(context, xc, yc, x, y, color1, color2)
    x = x * c - y * s
    y = x * s + y * c
  }
}
...
// Now DDA.
const circleDDA = (context, xc, yc, r, color, color2) => {
  const epsilon = 1 / r
  let x = r
  let y = 0

  while (x >= y) {
    plotCirclePoints(context, xc, yc, x, y, color, color2)
    x = x - epsilon * y
    y = y + epsilon * x
  }
}
...
const circleBres1 = (context, xc, yc, r, color, color2) => {
  let p = 3 - 2 * r
  let x = 0
  let y = r
...
// And another...
const circleBres2 = (context, xc, yc, r, color, color2) => {
  let x = 0
  let y = r
  let e = 1 - r
  let u = 1
...
// Last but not least...
const circleBres3 = (context, xc, yc, r, color, color2) => {
  let x = r
  let y = 0
  let e = 0
...
const Edge = class {
  constructor(p1, p2) {
    ...
    this.topColor = p1.y < p2.y ? p1.rgb : p2.rgb
    this.bottomColor = p1.y >= p2.y ? p1.rgb : p2.rgb
...
const fillPolygon = (context, polygon, color = [0, 0, 0]) => {
   ...
   * @param {*} ratio
   */
  const averageColor = (color1, color2, ratio) =>
    addArrays(
      color1,
      addArrays(
        color2,
        color1.map(x => -x)
      ).map(element => element * ratio)
    )
  ...
  while (activeEdgeList.length) {
    ...
    for (let i = 0, max = activeEdgeList.length; i < max; i += 1) {
      ...
      if (drawPixel) {
        ...
        let color1 = averageColor(
          activeEdgeList[i - 1].topColor,
          activeEdgeList[i - 1].bottomColor,
          (currentScanLine - activeEdgeList[i - 1].minY) / (activeEdgeList[i - 1].maxY - activeEdgeList[i - 1].minY)
        )
        let color2 = averageColor(
          activeEdgeList[i].topColor,
          activeEdgeList[i].bottomColor,
          (currentScanLine - activeEdgeList[i].minY) / (activeEdgeList[i].maxY - activeEdgeList[i].minY)
        )

        // No cheating here --- draw each pixel, one by one.
        for (let x = fromX; x <= toX; x += 1) {
          setPixel(context, x, currentScanLine, ...averageColor(color1, color2, (x - fromX) / (toX - fromX)))
        }
      } else {
