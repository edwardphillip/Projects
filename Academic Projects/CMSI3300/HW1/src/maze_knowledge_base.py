...
class MazeKnowledgeBase:
    ...
    def ask(self, query: "MazeClause") -> bool:
        ...
        for prop in query.props:
            negate_query_arr.append((prop, not query.get_prop(prop)))

        negate_query: object = MazeClause(negate_query_arr)
        
        clauses.add(negate_query)
        ...
        while len(clauses) > 0:
            for c1, c2 in itertools.combinations(clauses, 2):
                ...
                for clause in resolvents:
                    if clause.is_empty():
                        return True
                    new_set = new_set.union(resolvents)
            if new_set.issubset(clauses):
                return False
