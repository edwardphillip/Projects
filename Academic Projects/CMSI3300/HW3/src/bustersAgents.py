    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhosts:
            if ghost:
                ...
                index += 1
                best_position, max_prob = pacmanPosition, 0
                for pos, val in distribution.items():
                    if val > max_prob:
                        max_prob = val
                        best_position = pos
                ghosts_positions.append(best_position)

        least_distance = float("inf")
        closest_ghost = pacmanPosition
        ...
        go_action = legal[0]
        current_distance = self.distancer.getDistance(pacmanPosition, closest_ghost)
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            potential_distance = self.distancer.getDistance(successorPosition, closest_ghost)
